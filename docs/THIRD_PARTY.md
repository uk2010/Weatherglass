...
The Weatherglass launcher icon is the original project artwork in `icon.png`, resized into the packaged hicolor sizes. It contains no Apple marks or copied symbols and is distributed with Weatherglass under GPL-3.0-or-later. The condition-aware scenic hero images in `data/hero/` are original project artwork made for this application; they contain no third-party or Apple assets. Apple’s Weather attribution mark is not packaged: it is fetched at runtime from WeatherKit’s official attribution response and remains Apple property.
